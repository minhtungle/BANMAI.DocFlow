...
using Index_OutPut_Dto_AIBot = Applications.QuanLyAIBot.Dtos.Index_OutPut_Dto;
...
{
    public class QuanLyAIBotService : BaseService, IQuanLyAIBotService
    {
        // Repo
        ...
        public async Task<Index_OutPut_Dto_AIBot> Index_OutPut()
        {
            var thaoTacs = GetThaoTacs(maChucNang: "QuanLyAIBot");

            return new Index_OutPut_Dto_AIBot
            {
                ThaoTacs = thaoTacs,
            };
        }
        ...
        public async Task<string> TaoNoiDungAI(TaoNoiDungAI_Input_Dto input)
        {
            ...
            try
            {
                string noiDung = await _quanLyAIToolAppService.WorkWithAITool(input: new WorkWithAITool_Input_Dto
                {
            ...
            {
                throw new Exception("Lỗi khi tạo nội dung AI: " + ex.Message);
            }
        }
        public async Task<string> TestNoiDungAI(TestNoiDungAI_Input_Dto input)
        {
            var aiBot  = await _aiBotRepo.GetByIdAsync(input.IdAIBot);
            aiBot.Prompt += string.Format("{0}: {1}",
                  "[THÔNG TIN CUNG CẤP] (nếu không có gì thì bỏ qua)",
                  input.Keywords);
            try
            {
        ...
        public async Task<DisplayModel_CRUD_AIBot_Output_Dto> DisplayModel_CRUD_AIBot_Output(
            DisplayModel_CRUD_AIBot_Input_Dto input)
        {
            var aiBot = await GetAIBots(loai: "single", idAIBots: new List<Guid> { input.IdAIBot });
            var loaiAIBot = await GetLoaiAIBots(loai: "all");
            ...
            {
                Loai = input.Loai,
                LoaiAIBot = loaiAIBot.ToList() ?? new List<tbLoaiAIBot>(),
        ...
        public async Task<DisplayModel_Test_AIBot_Output_Dto> DisplayModel_Test_AIBot_Output(
            Guid input)
        {
            var aiBot = await GetAIBots(loai: "single", idAIBots: new List<Guid> { input });
            var loaiAIBot = await GetLoaiAIBots(loai: "all");
            var aiTools = await _aiToolRepo.Query()
                .Where(x => x.TrangThai != 0 && x.MaDonViSuDung == CurrentDonViSuDung.MaDonViSuDung)
                .ToListAsync();
            var output = new DisplayModel_Test_AIBot_Output_Dto
            {
                LoaiAIBot = loaiAIBot.ToList() ?? new List<tbLoaiAIBot>(),
                AIBot = aiBot.FirstOrDefault() ?? new tbAIBotExtend()
                {
        ...
        public async Task Delete_AIBot(List<Guid> idAIBots)
        {
            await _unitOfWork.ExecuteInTransaction(async () =>
            {
                var aiBots_DELETE = await _aiBotRepo.Query()
                    .Where(x => idAIBots.Contains(x.IdAIBot))
                    .ToListAsync();
                foreach (var entity in aiBots_DELETE)
                {
                    entity.TrangThai = 0;
                    entity.NgaySua = DateTime.Now;
                    entity.IdNguoiSua = CurrentNguoiDung.IdNguoiDung;
                ...
                var map_DELETE = await _aiBotLoaiAIBotRepo.Query()
                    .Where(x => idAIBots.Contains(x.IdAIBot.Value))
                    .ToListAsync();
                foreach (var entity in map_DELETE)
                {
                    await _unitOfWork.DeleteAsync<tbAIBotLoaiAIBot, Guid>(entity);
                }
            });
        }
        #endregion
        ...
        public async Task<DisplayModel_CRUD_LoaiAIBot_Output_Dto> DisplayModel_CRUD_LoaiAIBot_Output(
           DisplayModel_CRUD_LoaiAIBot_Input_Dto input)
        {
            var loaiAIBot = await GetLoaiAIBots(loai: "single", idLoaiAIBots: new List<Guid> { input.IdLoaiAIBot });
        ...
        public async Task Update_LoaiAIBot(tbLoaiAIBot loaiAIBot)
        {
            await _unitOfWork.ExecuteInTransaction(async () =>
            {
                var entity = await _loaiAIBotRepo.GetByIdAsync(loaiAIBot.IdLoaiAIBot);

                if (entity == null)
                    throw new Exception("AI Bot không tồn tại.");

                // Cập nhật thông tin chính
                entity.TenLoaiAIBot = loaiAIBot.TenLoaiAIBot;
                entity.GhiChu = loaiAIBot.GhiChu;
                entity.NgaySua = DateTime.Now;
                entity.IdNguoiSua = CurrentNguoiDung.IdNguoiDung;
                ...
                // Xóa liên kết loại cũ
                var oldMappings = await _aiBotLoaiAIBotRepo.Query()
                    .Where(x => x.IdLoaiAIBot == entity.IdLoaiAIBot)
                    .ToListAsync();
                foreach (var old in oldMappings)
                {
                    await _unitOfWork.DeleteAsync<tbAIBotLoaiAIBot, Guid>(old);
        ...
        public async Task Delete_LoaiAIBot(List<Guid> idLoaiAIBots)
        {
            await _unitOfWork.ExecuteInTransaction(async () =>
            {
                var loaiAIBots_DELETE = await _loaiAIBotRepo.Query()
                    .Where(x => idLoaiAIBots.Contains(x.IdLoaiAIBot))
                    .ToListAsync();

                foreach (var entity in loaiAIBots_DELETE)
                {
                    entity.TrangThai = 0;
                    entity.NgaySua = DateTime.Now;
                    entity.IdNguoiSua = CurrentNguoiDung.IdNguoiDung;
                ...
                var map_DELETE = await _aiBotLoaiAIBotRepo.Query()
                  .Where(x => idLoaiAIBots.Contains(x.IdLoaiAIBot.Value))
                  .ToListAsync();
                foreach (var entity in map_DELETE)
                {
                    await _unitOfWork.DeleteAsync<tbAIBotLoaiAIBot, Guid>(entity);
