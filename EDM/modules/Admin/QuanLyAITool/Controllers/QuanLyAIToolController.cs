...
namespace QuanLyAITool.Controllers
{
    ...
    public class QuanLyAIToolController : Controller
    {
        ...
        public readonly IQuanLyAIToolService _quanLyAIToolAppService;
        public QuanLyAIToolController(IQuanLyAIToolService quanLyAIToolAppService)
        {
        ...
        {
            var aiTool = await _quanLyAIToolAppService.GetAITools(loai: "single", idAITools: new List<Guid> { input.IdAITool });
        ...
        [HttpPost]
        public async Task<ActionResult> update_AITool()
        {
            try
            {
                var aiTool_NEW = JsonConvert.DeserializeObject<tbAITool>(Request.Form["aiTool"]);
                ...
                await _quanLyAIToolAppService.Update_AITool(aiTool: aiTool_NEW);
        ...
        [HttpPost]
        public async Task<ActionResult> delete_AITool()
        {
            try
            {
                var idAITools_NEW = JsonConvert.DeserializeObject<List<Guid>>(Request.Form["idAITools"]);
                if (idAITools_NEW == null)
                    return Json(new { status = "error", mess = "Chưa có bản ghi nào" }, JsonRequestBehavior.AllowGet);

                await _quanLyAIToolAppService.Delete_AITool(idAITools: idAITools_NEW);
                return Json(new { status = "success", mess = "Cập nhật thành công" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
