using Applications.QuanLyAITool.Enums;
...
{
    public class QuanLyAIToolService : BaseService, IQuanLyAIToolService
    {
        private readonly IRepository<tbAITool, Guid> _aiToolRepo;

        public QuanLyAIToolService(
            ...
            LocThongTinDto locThongTin = null)
        {
            ...
            if (loai == "single" && idAITools != null)
            {
                query = query.Where(x => idAITools.Contains(x.IdAITool));
            }
            ;
        ...
        public async Task<bool> IsExisted_AITool(tbAITool aiTool)
        {
            var aiTool_OLD = await _aiToolRepo.Query()
                .FirstOrDefaultAsync(x => x.ToolName == aiTool.ToolName
        ...
        public async Task Update_AITool(tbAITool aiTool)
        {
            await _unitOfWork.ExecuteInTransaction(async () =>
            {
                // 1. Mã hóa API Key
                ...
                // 4. Tạo entity
                var entity = await _aiToolRepo.GetByIdAsync(aiTool.IdAITool);
                if (entity == null)
                    throw new Exception($"Không tìm thấy AI Tool với Id: {aiTool.IdAITool}");
                entity.ToolCode = aiTool.ToolCode;
                entity.ToolName = aiTool.ToolName;
                entity.ApiEndpoint = aiTool.ApiEndpoint;
                entity.Model = aiTool.Model;
                entity.APIKey = encrypted;
                entity.IsEncrypted = true;
                entity.AdditionalHeaders = aiTool.AdditionalHeaders;
                entity.RequestBodyTemplate = normalizedTemplate;
                entity.GhiChu = aiTool.GhiChu;

                entity.IdNguoiSua = CurrentNguoiDung.IdNguoiDung;
                entity.NgaySua = DateTime.Now;

                // 5. Lưu
                await _unitOfWork.UpdateAsync<tbAITool, Guid>(entity);
            });
        }
        public async Task Delete_AITool(List<Guid> idAITools)
        {
            await _unitOfWork.ExecuteInTransaction(async () =>
            {
                var aiTools_DELETE = await _aiToolRepo.Query()
                    .Where(x => idAITools.Contains(x.IdAITool))
                    .ToListAsync();

                foreach (var entity in aiTools_DELETE)
                {
                    entity.TrangThai = 0;
                    entity.NgaySua = DateTime.Now;
                    entity.IdNguoiSua = CurrentNguoiDung.IdNguoiDung;
